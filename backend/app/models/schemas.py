...
class TripRequest(BaseModel):
    ...
    origin: str = Field(
        default="",
        description="出发地",
        json_schema_extra={"example": "上海"},
    )
    city: str = Field(default="", description="目的地城市(单城市兼容)", example="北京")
    cities: List[CityStay] = Field(default_factory=list, description="多城市行程配置")
...
class Attraction(BaseModel):
    ...
    image_source: Optional[str] = Field(default="", description="图片来源")
    image_author: Optional[str] = Field(default="", description="图片作者")
    image_license: Optional[str] = Field(default="", description="图片许可证")
    image_source_page: Optional[str] = Field(default="", description="图片来源页")
    image_attribution: Optional[str] = Field(default="", description="图片署名文本")
...
class TripPlan(BaseModel):
    ...
    cities: List[str] = Field(default_factory=list, description="所有途经城市列表")
...
class KnowledgeGraphData(BaseModel):
    ...
    categories: List[GraphCategory] = Field(default_factory=list, description="分类列表")
...
class TripChatRequest(BaseModel):
    ...
    history: Optional[List[ChatMessage]] = Field(default_factory=list, description="历史对话记录")
