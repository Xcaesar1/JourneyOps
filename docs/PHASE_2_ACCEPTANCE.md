...
| Cancel, retry, idempotency | V2 actions and DB unique constraints | API/repository tests |
| Worker/API restart | startup recovery and independent DB sessions | policy tests and staging restart drill |
...
| --- | --- |
| Local Ruff | 两组检查均通过 |
| Local pytest | `35 passed, 2 skipped`；跳过项为需要真实 PostgreSQL/Redis 的集成测试 |
...
| Cancellation and idempotency | 取消后保持 `cancelled`；重复幂等键返回同一任务 |
| Worker restart policy | 尝试次数耗尽的遗留 `processing` 任务恢复为 `failed/worker_lost` |
...
- Worker 被强制终止后，恢复延迟受 Redis visibility timeout、任务锁 TTL 和 stale threshold 影响。
- 阶段 2 不迁移旧 JSON 历史数据；旧文件保留但不再读取或写入。
